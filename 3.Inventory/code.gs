...
function doGet(e) {
  ...
  return HtmlService.createTemplateFromFile('app').evaluate()
      .setTitle('Inventory Smart App V.6.7.1')
...
function parsePickingList(text) {
  ...
  if (regionMatch) {
    ...
    // ค้นหา Region ที่เป็น standalone words ในข้อความ
  ...
  // NEW: ถ้ายังไม่พบ Region ให้ลองดักจาก DUID หรือ ค้นหาจาก Master Data
  if (header.region === "-" || header.region === "") {
    // ลองดูว่า DUID ลงท้ายด้วย Region หรือไม่ (เช่น SHSSM_..._ER)
    ...
    if (regions.indexOf(lastPart) > -1) {
      ...
      // ถ้ายังไม่เจออีก ให้ลองไปหาใน Sheet "data" (Master Data)
      try {
...
function runGasSystemTests() { var VERCEL_URL = 'https://project-ju28a.vercel.app'; var mockData = { header: { customer: "AIS", type: "IN", region: "ER", duid: "TEST-DUID-999", billNo: "TEST-BILL-001", ownerWarehouse: "TEST_WH", ownerReceiver: "TEST_RC" }, items: [{ type: "AISG", model: "AISG 3m", code: "4070193", desc: "Test Item", qty: 1, sn: "TEST-SN-001" }] }; Logger.log("🚀 เริ่มการทดสอบระบบ (GAS Version)..."); try { var opt1 = { method: 'post', contentType: 'application/json', payload: JSON.stringify(mockData), muteHttpExceptions: true }; var res1 = UrlFetchApp.fetch(VERCEL_URL + '/notify', opt1); Logger.log("1️⃣ ผลการทดสอบแจ้งเตือน: " + res1.getContentText()); } catch (e) { Logger.log("❌ Error 1: " + e); } try { var duidPayload = { message: { chat: { id: 7378939928 }, text: "SHSSM_2025 Coverage Expansion Ph1_New Site_Remote-BBU_ER" } }; var opt2 = { method: 'post', contentType: 'application/json', payload: JSON.stringify(duidPayload), muteHttpExceptions: true }; var res2 = UrlFetchApp.fetch(VERCEL_URL + '/telegram-webhook', opt2); Logger.log("2️⃣ ผลการทดสอบค้นหา DUID: " + res2.getContentText()); } catch (e) { Logger.log("❌ Error 2: " + e); } try { var billResult = searchByBillNo("TEST-BILL-001", "AIS"); Logger.log("3️⃣ ผลการทดสอบค้นหา Bill (Internal): " + JSON.stringify(billResult)); } catch (e) { Logger.log("❌ Error 3: " + e); } Logger.log("🏁 จบการทดสอบ ตรวจสอบผลลัพธ์ใน Telegram/LINE"); }
function testOcrEngine() { var mockBase64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+ip1sAAAAASUVORK5CYII="; Logger.log("🔍 กำลังทดสอบ Drive API OCR..."); try { var result = processOCR(mockBase64); Logger.log("✅ Drive API Status: " + (result.success ? "พร้อมใช้งาน" : "มีปัญหา")); if (!result.success) Logger.log("❌ Error: " + result.error); var mockText = "Bill No: T0826-TEST-001\nDUID: SHSSM_TEST_SITE_2025\nRegion: ER\nBBU5900 1\nUBBPg2 2"; var parsed = parsePickingList(mockText); Logger.log("📝 ผลการ Parsing จำลอง:"); Logger.log("🆔 DUID: " + parsed.header.duid); Logger.log("📄 Bill: " + parsed.header.billNo); Logger.log("📦 รายการ: " + parsed.items.length + " รายการ"); } catch (e) { Logger.log("❌ CRITICAL ERROR: " + e.toString()); Logger.log("💡 คำแนะนำ: ตรวจสอบว่าในเมนู Services (+) ทางซ้ายมือ ได้เพิ่ม 'Drive API' หรือยัง"); } }
