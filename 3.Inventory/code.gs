...
function updateDuidStatus(duid, customer) { try { if (!duid) return; var ss = SpreadsheetApp.openById(SPREADSHEET_ID); var sheet = ss.getSheetByName("INOUT_HW_" + customer); if (!sheet) return; var data = sheet.getDataRange().getValues(); var h = data[0].map(v => String(v||"").toUpperCase()); var idx = { duid: Math.max(h.indexOf("DUID"), 1), type: Math.max(h.indexOf("IN/OUT"), 3), qty: Math.max(h.indexOf("SUM OF REQ.QTY"), 10), status: Math.max(h.indexOf("STATUS"), 21) }; var inQty = 0, outQty = 0, dismantleQty = 0, returnQty = 0, strInQty = 0, strOutQty = 0, matchingRows = [], target = duid.trim(); for (var i = 1; i < data.length; i++) { if (String(data[i][idx.duid]).trim() === target) { matchingRows.push(i + 1); var type = String(data[i][idx.type]).toUpperCase().trim(), qty = Number(data[i][idx.qty]) || 0; if (type === "IN") inQty += qty; else if (type === "OUT") outQty += qty; else if (type === "DISMANTLE") dismantleQty += qty; else if (type === "RETURN") returnQty += qty; else if (type === "STR/IN") strInQty += qty; else if (type === "STR/OUT") strOutQty += qty; } } var totalItems = inQty + outQty + dismantleQty + returnQty + strInQty + strOutQty; var isBalanced = (inQty === outQty) && (dismantleQty === returnQty) && (strInQty === strOutQty); var status = "Pending"; if (totalItems > 0) { status = isBalanced ? "Closed" : "On Process"; } if (matchingRows.length > 0) { var statusRange = sheet.getRange(1, idx.status + 1, data.length, 1); var statusValues = statusRange.getValues(); matchingRows.forEach(r => { if (statusValues[r-1]) statusValues[r-1][0] = status; }); statusRange.setValues(statusValues); } } catch (e) {} }
