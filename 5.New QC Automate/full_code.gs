...
const REFERENCE_FOLDER_ID = "1rXNAuOTHL396QAiUgV6D88YgUJCaODom";
...
function listFilesInFolder(id) { try { const folder = DriveApp.getFolderById(id); const files = folder.getFiles(); const res = []; while (files.hasNext()) { const f = files.next(); if (!(f.getDescription() || "").includes("PAT_CHECKED")) res.push({ id: f.getId(), name: f.getName(), size: (f.getSize() / 1024).toFixed(0) + " KB" }); } return { folderName: folder.getName(), files: res }; } catch (e) { return { error: e.toString() }; } }
...
function findReferenceImage(folder, keyword) {
  try {
    const files = folder.getFiles();
    while (files.hasNext()) {
      const file = files.next();
      if (file.getMimeType().includes('image/') && file.getName().toLowerCase().includes(keyword.toLowerCase())) {
        return file;
      }
    }
    const subfolders = folder.getFolders();
    while (subfolders.hasNext()) {
      const found = findReferenceImage(subfolders.next(), keyword);
...
function getReferenceImageBase64(categoryName) {
  try {
    const root = DriveApp.getFolderById(REFERENCE_FOLDER_ID);
    const file = findReferenceImage(root, categoryName);
    if (file) {
    ...
    console.warn("Ref Image Search Error:", e);
  }
  return null;
}
...
function analyzeAI(file, customChecklist) {
  ...
  CRITICAL RULES:
  1. 'majorCategory', 'subCategory', and 'detail' MUST EXACTLY MATCH one of the items in the Hierarchy Checklist below.
  2. 'imageType' MUST be "before" or "after".
  
  Output MUST be valid JSON ONLY:
  {"majorCategory":"...","subCategory":"...","detail":"...","imageType":"before"|"after"}
  ...
  const finalResult = {
    ...
    subCategory: String(classification.subCategory || "General"),
    detail: String(classification.detail || "Misc"),
    imageType: String(classification.imageType || "unknown")
  };
